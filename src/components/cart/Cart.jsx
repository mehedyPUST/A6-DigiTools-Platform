        ...
        <div className="w-11/12 mx-auto">
            ...
            {cart.length === 0 ? (
                ...
                <div className="space-y-4  ">
                    ...
                    {cart.map(item => (
                        <div key={item.id} className="shadow-xl   bg-white rounded-md py-2 px-3 flex items-center justify-between">
                            <div className="flex gap-2 items-center">
                                <img className="w-10 h-10  shadow-sm rounded-md" src={item.icon} alt={item.name} />
                                <div>
                            ...
                            <button onClick={() => removeFromCart(item.id, item.name)}
                                className=" text-[#FF3980] cursor-pointer text-sm md:text-base"> Remove</button>
                        </div>
                    ))}
                    <hr className='border-purple-600 mt-4 ' />
